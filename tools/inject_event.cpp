...
int main(int argc, char* argv[]) {
    ...
    // client presents the token that was generated by mesh_manager.py
    // and written to IPC_TOKEN_FILE at boot.
    {
        std::ifstream token_file(IPC_TOKEN_FILE);
        if (!token_file.is_open()) {
            std::cerr << "[SIM] Token file " << IPC_TOKEN_FILE
                      << " not found. Is the mesh running?" << std::endl;
            close(fd);
            return 1;
        }
        ...
        while (!token.empty() && (token.back() == '\n' || token.back() == '\r' ||
            ...
            token.pop_back();
        if (token.empty()) {
            std::cerr << "[SIM] Token file is empty." << std::endl;
            close(fd);
            return 1;
        }

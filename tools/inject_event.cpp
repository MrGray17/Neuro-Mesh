...
static const char* IPC_TOKEN_FILE = "/tmp/neuro_mesh_token";
...
int main(int argc, char* argv[]) {
    ...
    // IPC token auth handshake. The node refuses commands unless the
    // client presents the token that was generated at boot. If no
    // token file exists, skip auth — the node may be running without
    // IPC token (e.g. Docker containers without mesh_manager.py).
    {
        std::ifstream token_file(IPC_TOKEN_FILE);
        if (token_file.is_open()) {
            std::string token((std::istreambuf_iterator<char>(token_file)),
                              std::istreambuf_iterator<char>());
            while (!token.empty() && (token.back() == '\n' || token.back() == '\r' ||
                   token.back() == ' ' || token.back() == '\0'))
                token.pop_back();
            if (token.empty()) {
                std::cerr << "[SIM] Token file is empty. Connecting without auth." << std::endl;
            } else {
                std::string auth = "AUTH " + token + "\n";
                ssize_t aw = write(fd, auth.data(), auth.size());
                if (aw < 0) {
                    std::cerr << "[SIM] Failed to send AUTH handshake." << std::endl;
                    close(fd);
                    return 1;
                }
            }
